...
void decode_processing(byte const *instr, unsigned int const *registers, Decoded_Instruction *decoded_instr) {
    ...
    decoded_instr->opcode = ((instr[1] & GET_BIT_1) << 3) | (instr[2] >> 1);
    decoded_instr->S = instr[2] & GET_BIT_1;
    decoded_instr->Rn = instr[3];
    decoded_instr->Rd = instr[4];
    byte I = (instr[1] >> 1) & GET_BIT_1;
    if (I) {
        ...
        decoded_instr->carry = ((decoded_instr->operand2) >> ((instr[5] << 1) - 1)) & GET_BIT_1;
        ...
        byte shift_type = (instr[6] >> 1) & GET_FIRST_2_BITS;
        byte shift_by;
        byte bit4 = instr[6] & GET_BIT_1;
        if (bit4) {
            // If (bit4 == 1) shift by the last byte of a register
            shift_by = registers[instr[5]] & GET_FIRST_8_BITS;
        } else {
...
void decode_multiplying(byte const *instr, Decoded_Instruction *decoded_instr) {
    ...
    decoded_instr->A = (instr[2] >> 1) & GET_BIT_1;
...
void decode_transferring(byte const *instr, unsigned int const *registers, Decoded_Instruction *decoded_instr) {
    ...
    decoded_instr->L = instr[2] & GET_BIT_1;
    decoded_instr->P = instr[1] & GET_BIT_1;
    decoded_instr->U = (instr[2] >> 3) & GET_BIT_1;
    decoded_instr->Rn = instr[3];
    decoded_instr->Rd = instr[4];
    byte I = (instr[1] >> 1) & GET_BIT_1;
    if (I) {
